/********************************************************************************
	File: 		System/Cryptography/RijndaelProvider.js
	Version: 	1.0.0
	Date:		2003-12-15
...
Cryptography.RijndaelProvider = function() {
	Cryptography.ISymmetricProvider.call(this) ;
	...
	this.Encrypt = function(key, plaintext, mode) {
		...
		var ct;                                 // ciphertext
		if (!plaintext || !key)	
			throw new ArgumentException("RijndaelProvider.Encrypt: You must provide both the plaintext and the key.") ;
		if (key.length*8 != keySizeInBits) 
			throw new ArgumentException("RijndaelProvider.Encrypt: the length of the key should be " + keySizeInBits / 8 + " bits.") ;
		if (!mode) var mode = Cryptography.CipherMode.ECB ;

		if (mode == Cryptography.CipherMode.CBC) ct = getRandomBytes(bpb);
		else ct = new Array();
		...
		for (var block = 0; block < plaintext.length / bpb; block++) {
			...
			if (mode == Cryptography.CipherMode.CBC){
	...
	this.Decrypt = function(key, ciphertext, mode) {
		...
		if (!ciphertext || !key) 
			throw new ArgumentException("RijndaelProvider.Decrypt: You must provide both the ciphertext and the key.") ;
		if (ciphertext.IsInstanceOf(String)) 
			throw new ArgumentException("RijndaelProvider.Decrypt: the ciphertext must be passed in the form of a byte array.") ;
		if (key.length*8 != keySizeInBits) 
			throw new ArgumentException("RijndaelProvider.Decrypt: the length of the key should be " + keySizeInBits / 8 + " bits.") ;
		if (!mode) var mode = Cryptography.CipherMode.ECB ;
		expandedKey = keyExpansion(key);
		...
		if (mode == Cryptography.CipherMode.ECB) 
